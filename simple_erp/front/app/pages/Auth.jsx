...
export function Auth(){
    ...
    function checkFields(){
        if(email && password && document.getElementById('error-box').innerHTML == ''){
            setCanSend(true);
        }
    }
    ...
    return (
        ...
        <div className='flex justify-center flex-col items-center'>
            <div className='flex p-5 justify-center flex-col items-center gap-5 bg-gray-800 border-2 dark:border-white w-2xs'>
                ...
                <input type="email" defaultValue={''} name='email' onChange={(e) => {
                    changeEmail(e.target.value);
                }} className='bg-white rounded-md text-black p-1 border-2 border-black dark:border-0' />
                <label htmlFor="password">Password:</label>
                <input type="password" defaultValue={''} name='password' onChange={(e) => {
                    changePassword(e.target.value);
                }} className='bg-white rounded-md text-black p-1 border-2 border-black dark:border-0' />
                <input type="button" value="Отправить данные" className='bg-emerald-600 p-2 rounded-md hover:cursor-pointer' onClick={async () => {
                    checkFields();
