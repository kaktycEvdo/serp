...
export function Reg(){
    ...
    function changeOrgName(org){
        if(org.trim() != ''){
            if(org.length == 0){
                document.getElementById('error-box').innerHTML = 'Пустое наименование организации';
                setCanSend(false);
            }
            else{
                setOrgName(org);
    ...
    function checkFields(){
        if(email && password && create_new_org && org_name && document.getElementById('error-box').innerHTML == ''){
            setCanSend(true);
        }
    }
    ...
    return (
        <div className='grid lg:grid-cols-2 sm:grid-cols-1 justify-center items-center gap-5 p-5'>
            <h1 className='break-words lg:col-start-1 lg:col-end-3 text-center'>Регистрация</h1>
            <div className='grid h-96 overflow-y-scroll overflow-x-hidden p-5 justify-center flex-col items-center lg:w-96 gap-5 bg-gray-800 border-2 dark:border-white w-2xs'>
                ...
                <div className='flex max-h-full p-5 justify-center flex-col items-center gap-5'>
                    <div className='flex justify-center flex-col items-center gap-5'>
                        <p className='text-center'>Вы представитель уже существующей в системе организации или новой?</p>
                        <div className='flex gap-5 w-full justify-between'>
                            <label htmlFor="create_org_no" className='text-left'>Существующей</label>
                            <input type="radio" defaultChecked={true} name="create_org" id="create_org_no" value={0} onChange={(e) => {
                            setCNO(e.target.value);
                            }} />
                        </div>
                        <div className='flex gap-5 w-full justify-between'>
                            <label htmlFor="create_org_yes">Новой</label>
                            <input type="radio" name="create_org" id="create_org_yes" value={1} onChange={(e) => {
                            setCNO(e.target.value);
                            }} />
                        </div>
                        <div>
                            <label htmlFor="org_name">Наименование организации:</label>
                            <input type="text" defaultValue={''} id='org_name' className='bg-white border-2 w-full border-black rounded-md p-2 text-black' onChange={(e) => {
            ...
            <div className='lg:col-start-1 lg:col-end-3 justify-center items-center flex'>
                <input type="button" className='bg-emerald-600 p-2 rounded-md hover:cursor-pointer' value="Отправить данные" onClick={() => {
                    checkFields();
